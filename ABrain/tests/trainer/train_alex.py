...
ds = ILSVRC2010Train("/home/zhanghongda/datasets/ILSVRC2010/")
print(len(ds))
gpu = torch.device("cuda:3")
model = AlexNet(1000, 3).to(gpu)
...
model.train()
for e in range(100):
    dataloader = tqdm.tqdm(dl)
    cnt = 0
    for img, lab in dataloader:
        img = img.to(gpu)
        lab = lab.to(gpu)
        pre = model(img)
        loss = loss_fun(pre, lab)
        dataloader.set_description("loss:%.6f" % (loss.item()))
        opt.zero_grad()
        loss.backward()
        opt.step()
